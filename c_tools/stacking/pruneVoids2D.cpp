...
int main(int argc, char **argv) {
  ...
  volNorm = numPartTot/(boxLen[0]*boxLen[1]);
  printf(" VOL NORM = %f\n", volNorm);
  ...
  while (fgets(line, sizeof(line), fp) != NULL) {
    ...
    voids[i-1].radius = pow(voidVol/volNorm/M_PI, 1./2.);
    ...
    voids[i-1].eval = gsl_vector_alloc(2);
    voids[i-1].evec = gsl_matrix_alloc(2,2);
    voids[i-1].ellip = 0;
  }
  fclose(fp);
...
//    }
    ...
    for (int p = 0; p < voids[iVoid].numPart; p++) {
      ...
      for (int i = 0; i < 2; i++) {
      for (int j = i; j < 2; j++) {
        if (i == j) inertia[i*2+j] += dist[0]*dist[0] + dist[1]*dist[1];
        inertia[i*2+j] -= dist[i]*dist[j];      
      }
      }
    }
    inertia[2] = inertia[1];
